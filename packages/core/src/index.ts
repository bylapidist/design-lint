export { Linter, type Config, applyFixes } from './core/linter.js';
export { Runner } from './core/runner.js';
export type { DocumentSource } from './core/document-source.js';
export { FileSource } from './core/file-source.js';
export { builtInRules } from './rules/index.js';
export { loadCache, type Cache, type CacheEntry } from './core/cache.js';
export { getIgnorePatterns, loadIgnore, defaultIgnore } from './core/ignore.js';
export { realpathIfExists, relFromCwd } from './utils/paths.js';
export type {
  LintResult,
  LintMessage,
  RuleModule,
  RuleContext,
  RuleListener,
  DesignTokens,
  PluginModule,
  CSSDeclaration,
  Fix,
} from './core/types.js';
export {
  matchToken,
  closestToken,
  extractVarName,
  mergeTokens,
  normalizeTokens,
  type TokenPattern,
  type NormalizedTokens,
} from './core/token-utils.js';

...
function basePolicy(overrides: Partial<DesignLintPolicy> = {}): DesignLintPolicy {
  return {
    requiredRules: [],
    minSeverity: {},
    tokenCoverage: {},
...
test('loadPolicy throws ConfigError for invalid JSON', () => {
  const dir = makeTmpDir();
  fs.writeFileSync(path.join(dir, 'designlint.policy.json'), '{bad json}', 'utf8');
...
test('enforcePolicy passes when required rule is enabled', () => {
  ...
  assert.doesNotThrow(() => enforcePolicy(config, policy));
});
...
test('enforcePolicy throws when required rule is missing', () => {
  ...
  const policy = basePolicy({ requiredRules: ['color/no-raw-value'] });
  assert.throws(() => enforcePolicy(config, policy), ConfigError);
});
